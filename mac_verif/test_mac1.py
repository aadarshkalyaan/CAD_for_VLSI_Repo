...
from cocotb.triggers import RisingEdge
from cocotb_coverage.coverage import coverage_db
from model_mac import *  # Import sample_cases from model_mac
...
async def test_mac_coverage(dut):
    ...
    for a, b, c, s in a_cases,b_cases,c_cases,s_cases :
        ...
        assert dut.get_MAC.value == expected_mac, \
            f"MAC Output Mismatch in mode S{s}: Expected = {expected_mac}, Got = {dut.get_MAC.value}, A = {a}, B = {b}, C = {c}"
